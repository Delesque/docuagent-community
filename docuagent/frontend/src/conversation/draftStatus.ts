/** Per-turn draft status line shown while interviewing.
 *
 ...
 *  what their answer changed. This line reports counts only — never module names,
 *  never topology — so the user can see the draft grow and what still blocks
 *  completion without the draft being revealed early.
 *
 *  Zero-value parts are dropped: "0 条提醒" is noise, not reassurance. A turn with
 *  nothing to report returns null and no line is shown.
 */
...
export function formatDraftStatus(status: DraftStatus): string {
  ...
  // Scheme B: with a model-authored plan the progress speaks the plan's own
  // vocabulary; without one the fixed-topic fallback keeps the old display.
  if (status.aspects.length > 0) {
    const line = status.aspects
      .map((aspect) => `${aspect.title}(${aspect.asked}/${aspect.max})`)
      .join("、");
    const started = status.aspects.filter((aspect) => aspect.asked > 0).length;
    parts.push(`访谈方面 ${started}/${status.aspects.length} 已开始：${line}`);
  } else if (status.topicsRemaining.length > 0) {
    parts.push(`未覆盖主题：${status.topicsRemaining.join("、")}`);
  }
  return `当前草案：${parts.join(" · ")}`;
}
