...
export default function App() {
  ...
  useEffect(() => {
    ...
    // Build the full conversation from history + current message. Chips ride along so a
    // restored conversation keeps its buttons; an empty list is omitted rather than stored.
    ...
    for (const msg of history) {
      allMessages.push({
        ...
        chips: msg.chips?.length ? msg.chips : undefined,
      });
    }
    if (message) {
      allMessages.push({
        ...
        timestamp: new Date().toISOString(),
        chips: message.chips?.length ? message.chips : undefined,
      });
    }
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden">
      {architecture && projection === "outline" ? (
        ...
        /* No graph yet: the conversation owns the whole screen. The live draft status
           floats above it rather than inside the message, so it updates in place instead
           of being re-printed under every question. */
        <>
          <DraftStatusBar state={activeBootstrap} />
          {conversationSurface}
        </>
      )}
